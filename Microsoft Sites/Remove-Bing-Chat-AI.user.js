      ...
      mutations.forEach(function(mutation) {
          ...
          if (mutation.addedNodes.length > 0) {
            ...
            if (genIDs.length > 0){
              ...
              //partial matches for feeback buttons
           var feedback = document.querySelectorAll('[id^="fdbk"]');
              if (feedback.lengh > 0){
                  feedback.forEach(feedback =>{
                      removeElementById(feedback.id);
                  })
              }
                              //partial matches for more feeback buttons
           var feedbackOther = document.querySelectorAll('[id^="fdbkans_"]');
              if (feedbackOther.lengh > 0){
                  feedbackOther.forEach(feedbackOther =>{
                      removeElementById(feedbackOther.id);
                  })
              }
              ...
              // - // - // - //
              // ... More Section, remove Copilot search because ... wait, there's another copilot button to search? Why?
              removeElementById('b-scopeListItem-copilotsearch');
              // remove "create" in images, this is literally AI generation, it isnt even mentioned as so...
              removeElementById('imgcrtr_key');
              // Remove "inspire" button, I cant tell if it's AI for sure, but it does feel weird. It is circled like all their other AI tool options.
              // comment out the below to re-enable:
              removeElementById('imginspn_key');
              ...
              removeElementById('fbpgbt');
              removeElementById('b_footnote');
              //
              ...
              removeElementsByClassName('b_wpt_attr');
              removeElementsByClassName('fbans');
